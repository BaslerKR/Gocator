...
kDeclareValueEx(GoApi, GoDataTreeChunkedBinary, kValue)
...
public:
    ...
    /**
     * Adds a binary chunk to GoDataTree.
     *
     * @param bytes                             kArray1 of kBytes.
     *
     * @throw kERROR_PARAMETER if kArray1 data type is not kByte.
     * @throw kERROR_PARAMETER if the tree is not a chunked array
     * 
     * @code {.cpp}
     * kArray1 header = GetHeader();
     * kArray1 body = GetBody();
     * GoDataTree tree = GoDataTree::ChunkedBinary();
     * tree.AddChunk(header);
     * tree.AddChunk(body);
     * GoMsgPackFormat serialized;
     * GoDataTree::ToMsgPack(tree, serialized);
     * // header and body are now contiguous in serialized
     * @endcode
     */
    void AddChunk(kArray1 bytes);
    ...
    /**
     * Checks if GoDataTree item is a collection of kArray1 holding bytes. GoDataTree chunked binary represents contiguous binary data, which can be added in chunks.
     *
     * @return                                  True if GoDataTree item is chunked binary, false otherwise.
     */
    bool IsChunkedBinary() const;
    ...
    /**
     * Applies a merge patch to the current GoDataTree object.
     ...
     *       return Patch
     *   ```
     *   Where, `Target` is the current object; `Patch` is applyPatch argument.
     *
     * @param applyPatch                        A patch to apply. 
     */
    void MergePatch(const GoDataTree& applyPatch);
...
public:
    ...
    /**
     * Returns GoDataTree chunked binary.
     *
     * @return                                  GoDataTree chunked binary.
     */
    static GoDataTree ChunkedBinary();
     ...
     * @param fx                                Function to be executed to stream binary data.
     */
    static void ToMsgPack(const GoDataTree& tree, kObject out, k64u binarySize, StreamFx fx);
    ...
    /**
     * Calculates GoDataTree size in message pack format along with specifying binary size
     * to support streamed data whose binary size has to be pre-calculated.
     *
     * @param tree                              GoDataTree input.
     * @param binarySize                        Pre-calculated size of the binary in bytes.
     * @return                                  Message pack size.
     */
    static kSize CalculateMsgPackSize(const GoDataTree& tree, k64u binarySize);
...
typedef struct GoDataTreeChunkedBinary {} GoDataTreeChunkedBinary;
