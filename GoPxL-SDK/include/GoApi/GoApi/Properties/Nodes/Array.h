...
protected:
    ...
    // Events that fire when a child is added or removed.
    Event<Array<T>&, T&> onAddedEvent;
    Event<Array<T>&, T&> onRemovedEvent;
public:
    Array()
    {
    ...
    T& Emplace(Args&&... args)
    {
        ...
        T& childElement = *dynamic_cast<T*>(children.back().get());
        onAddedEvent.Notify(*this, childElement);
        return childElement;
    }
    ...
    void Reserve(size_t count, Args&&... args)
    {
        ...
        {
            // Emplace will notify onAddedEvent for any children added.
    ...
    virtual void Resize(size_t count) override
    {
        ...
        {
            // Notify onRemovedEvent for each child to be removed.
            for (size_t idx = count; idx < children.size(); idx++)
            {
                onRemovedEvent.Notify(*this, At(idx));
            }
    ...
    virtual void Remove(size_t index) override
    {
        ...
        onRemovedEvent.Notify(*this, At(index));
    ...
    T& Add(pointer_t newChild)
    {
        ...
        T& newChildElement = *dynamic_cast<T*>(children.back().get());
        onAddedEvent.Notify(*this, newChildElement);
    ...
    T& Insert(pointer_t newChild, size_t index)
    {
        ...
        T& newChildElement = *dynamic_cast<T*>(ret->get());
        onAddedEvent.Notify(*this, newChildElement);
        return newChildElement;
    }
    ...
    Event<Array<T>&, T&>& OnRemovedEvent()
    {
        return onRemovedEvent;
    }

    void RetrieveChangedParameters(std::vector <Go::Properties::Node*>& changedParams) override
    {
        std::vector <Go::Properties::Node*> tempChangedParams;

        for (size_t i = 0; i < this->Count(); i++)
        {
            this->At(i).RetrieveChangedParameters(tempChangedParams);
        }

        // If any of the internal contents of the array have changed, add the parent array parameter.
        if (tempChangedParams.size() > 0)
        {
    ...
    void ResetChangedParameters() override
    {
        for (size_t i = 0; i < this->Count(); i++)
        {
            this->At(i).ResetChangedParameters();
        }
    }
