...
public:
    ...
    // GOS-13008 Currently disabled as not needed but may be used in the future.
#ifdef ALLOW_PUBLIC_ACCESS_TO_HASCHANGED
    // Return whether this value has changed since the last time the flag was reset.
    bool ValueHasChanged()
    {
        return hasChanged;
    }

    // Reset the flag to allow detection of a subsequent value change.
    void ResetValueHasChangedFlag()
    {
        hasChanged = false;
    }
#endif
    ...
    // This event fires after a params update, not right when the param changes.
    // Internal changes (via Value::Set or equivalent) will not trigger this event.
    Event<Value&>& OnUpdatedEvent()
    {
        return onUpdatedEvent;
    }

    // Implementation of virtual method from IValueIo
    void ReadValue(IValueReader& reader, Event<>& endReadEvent) override
    {
        T val;
        ValueIoDispatch<T>::Read(reader, val);
        if (val != Internal::ValueWrapper<T>::Get())
        {
            endReadEvent.AddListener(std::bind(&Value::OnUpdateEnd, this));
        }
        this->Set(val);
    }
