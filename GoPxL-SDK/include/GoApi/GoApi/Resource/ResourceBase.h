 ...
 */
     ...
     * The "allowSchema" lets the caller decide if having schema in the output is appropriate or not, even if the Node structure
     * inherently has schema information. This allows the caller to suppress output of schema, even if the
     * REST request asks for schema info.
     */
    void NodeToGoDataTree(const Go::Properties::Node& node, const RequestInfo& reqInfo, GoDataTree reply, bool allowSchema = true);
...
public:
    ...
    virtual void UpdateDocument(const RequestInfo& reqInfo, const GoDataTree& data);
};
