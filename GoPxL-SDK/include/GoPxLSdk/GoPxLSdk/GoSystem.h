...
public:
    ...
    /**
     * Gets the Gocator Data Protocol (GDP) port.
    ...
    /**
     * Returns the managed GoResource for the given URI. Creates one if it does
     * not yet exist. The caller receives shared ownership via std::shared_ptr.
     * Managed resources participate in non-idempotent invalidation and can be
     * subscribed for server notifications.
     *
     * @public                              @memberof GoSystem
     ...
     */
    std::shared_ptr<GoResource> Resource(const std::string& uri);

    /**
     * Returns a reference to the GoResourceManager for advanced management
     * of subscriptions and cache invalidation.
     *
     * @public                              @memberof GoSystem
     * @version                             Introduced in 1.5
     * @return                              GoResourceManager reference.
     */
    GoResourceManager& ResourceManager();

    /**
     * Registers a callback invoked when the connection to the sensor is lost.
     *
     * The callback fires on the channel's background I/O thread when a read
     * error is detected (e.g., TCP connection drop). It fires at most once
     * per connection session and resets on Connect(). Before the callback is
     * invoked, the GoResourceManager's caches are automatically invalidated
     * and subscription state is reset.
     *
     * @public                              @memberof GoSystem
     * @version                             Introduced in 1.5
...
private:
    ...
    std::unique_ptr<GoResourceManager> resourceManager_;

    kIpAddress address = { };
    k16u port = GO_PXL_SDK_DEFAULT_CONTROL_PORT;
    std::function<void()> userDisconnectHandler_;
