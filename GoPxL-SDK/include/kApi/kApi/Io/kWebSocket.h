 ...
 *
 * When a non-zero maximum message size is configured and a write operation would cause
 * the current message to exceed this limit, the current message is automatically ended
 * (sent) and the new data begins a new message. 
 * 
 * This setting applies only to the streaming write path (kStream_Write/kWebSocket_Send), 
 * not to kWebSocket_WriteMessage.
 * 
 * The maximum message size cannot be changed while a message is in progress (i.e., between
 * kStream_Write and kWebSocket_Send).
 *
 * By default, the maximum message size is 0 (unlimited).
 *
 * @public              @memberof kWebSocket
 * @param   webSocket   WebSocket object.
 * @param   size        Maximum message size in bytes (0 for unlimited).
 * @return              Operation status.
 */
kFx(kStatus) kWebSocket_SetMaxMessageSize(kWebSocket webSocket, kSize size);
 ...
 * @public              @memberof kWebSocket
 * @param   webSocket   WebSocket object.
 * @return              Maximum message size in bytes (0 if unlimited).
 */
kFx(kSize) kWebSocket_MaxMessageSize(kWebSocket webSocket);
