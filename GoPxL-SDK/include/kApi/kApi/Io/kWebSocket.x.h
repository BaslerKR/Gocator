...
typedef struct kWebSocketSendInfo
{
    ...
    kBool inMessage;                    //are we currently in a data frame sequence
    kSize messageBytesWritten;          //total payload bytes written as frames in the current message
} kWebSocketSendInfo;
...
typedef struct kWebSocketClass
{
    ...
    kSize maxMessageSize;               //maximum message size before auto-sending (0 = unlimited)
