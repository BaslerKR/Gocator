...
#   if defined(K_MSVC) && !defined(__clang__)
...
#   if defined (K_DARWIN)
#       define pthread_mutex_timedlock_(MUTEX, ABSTIME)         xkDarwin_pthread_mutex_timedlock(MUTEX, ABSTIME)
...
#       define clock_gettime_(ABSTIME)                          clock_gettime(CLOCK_REALTIME, ABSTIME)
        ...
        static inline int xkDarwin_pthread_mutex_timedlock(pthread_mutex_t* mutex, const struct timespec* abstime)
        {
            int rc;
...
#   elif defined (K_LINUX) && defined(__GLIBC_PREREQ)
