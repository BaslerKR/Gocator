...
kForwardDeclareClass(k, kStreamSizeCounter, kStream)
...
kDeclareValueEx(k, kAbgr, kValue)

kFx(kBool) xkAbgr_VEquals(kType type, const void* value, const void* other); 
kFx(kSize) xkAbgr_VHashCode(kType type, const void* value); 
kFx(kStatus) xkAbgr_Write(kType type, const void* values, kSize count, kSerializer serializer);
kFx(kStatus) xkAbgr_Read(kType type, void* values, kSize count, kSerializer serializer);
...
//FPT-2682: provided for compatibility with old code bases.
