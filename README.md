...
* **독립적 로깅 프레임워크**: `Gocator::syslog()`를 통해 외부 라이브러리(예: 호스트 애플리케이션의 로깅 엔진)가 리다이렉트하여 수집할 수 있도록 표준 스트림 기반 로깅을 제공합니다.
...
> [!NOTE]
> **호스트 로그 전달 계약 (`syslog`)**
> `Gocator::syslog()`는 단일 `[Gocator]` 레코드를 `std::cout` 또는 `std::cerr`로 즉시 플러시하여 출력합니다. 호스트 스트림 리다이렉터나 로깅 수집기는 버퍼링 대기 없이 라이프사이클 및 오퍼레이션 피드백을 수집할 수 있습니다.
